...
export const config = {
  ...
  // 22.08 вечер (мандат владельца «действуй как управляющий, делаем сразу»):
  // зеркала пересажены на перекалиброванные ячейки afks-meanrev2 / sibn-meanrev2
  // (старые на окне март→август минусовые); AFKS остаётся на 2 лотах —
  // возврат к 5 после exp14 > 0 новой ячейки
  mirrors: (process.env.MIRRORS || 'AFKS:afks-meanrev2:2,SIBN:sibn-meanrev2:5')
    .split(',')
    .map(s => {
