...
if (isMain) {
  (async () => {
    ...
    // Бонферрони по ПОЛНОМУ числу гипотез: двусторонний порог |t|
    const alpha = 0.05 / totalHypos;
    const tCrit = normalQuantile(1 - alpha / 2);
...
/** Квантиль стандартного нормального распределения (аппроксимация Аклама,
 *  устойчива в хвостах — ровно там живёт порог Бонферрони). */
function normalQuantile(p: number): number {
  ...
  const pl = 0.02425;
  if (p <= 0 || p >= 1) throw new Error(`normalQuantile: p=${p}`);
  if (p < pl) {
